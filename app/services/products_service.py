...
from app.clickhouse import DeliveryReport, get_delivery_client
...
async def do_delivery(db: Session, shop: Shop, admin: Employee, courier: Employee) -> None:
    global UPDATED_PRODUCTS
    
    delivery_products = db.query(Product, Category) \
    ...
    delivery_products = random.sample(delivery_products, max(1, random.randint(1, len(delivery_products))))
    ...
    for product, category in delivery_products:
        ...
        db.add(product)
        UPDATED_PRODUCTS.add(product.product_id)
        delivery_report.add_order_data(
    ...
    async with get_delivery_client() as client:
